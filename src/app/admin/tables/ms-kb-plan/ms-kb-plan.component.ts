...
export interface Plan {
  ...
  AnnualMax?;
  MinimumMonthlyFee?;
  OverKBRateMonthlyPlan?;
  OverKBRateAnnualPlan?;
  EffectiveKBRate?;
}

@Component({
  selector: 'app-ms-kb-plan',
  templateUrl: './ms-kb-plan.component.html',
  styleUrls: ['./ms-kb-plan.component.css'],
  providers: [AdminService, MessageService]
})
export class MsKbPlanComponent implements OnInit {
  ...
  constructor(private service: AdminService,
    ...
    this.cols = [
      ...
      { field: 'PlanNumber', header: 'PlanNo.' },
      { field: 'TransactionalPlan', header: 'Transactional Plan' },
      { field: 'MonthlyMax', header: 'Monthly Max' },
      { field: 'AnnualMax', header: 'Annual Max' },
      { field: 'MinimumMonthlyFee', header: 'Min Monthly Fee' },
      { field: 'OverKBRateMonthlyPlan', header: 'KB/Month' },
      { field: 'OverKBRateAnnualPlan', header: 'KB/Annual' },
      { field: 'EffectiveKBRate', header: 'Effective KBRate' }
    ];
  }
  ...
  delete() {
    if (confirm("Are you sure you want to delete this record?")) {
      this.service.deleteMsKbPlanId(this.selectedPlan.Id)
        .subscribe(data1 => {
