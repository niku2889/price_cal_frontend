    ...
    if (confirm("Are you sure you want to delete this record?")) {
      this.service.deleteDiametricsFeeId(this.selectedPlan.Id)
        .subscribe(data1 => {
          ...
          this.service.addLogs('DimetricsFees', this.selectedPlan.Id, localStorage.getItem('name'), localStorage.getItem('userId'), 'Delete',this.selectedPlan,null)
          .subscribe(l => {
          });
        }, err => {
    ...
    if (this.newPlan) {
      this.service.addDiametricsFee(this.plan)
        .subscribe(data1 => {
          ...
          this.service.addLogs('DimetricsFees', data1.Id, localStorage.getItem('name'), localStorage.getItem('userId'), 'Add',null, data1)
          .subscribe(l => {
          });
        }, err => {
          this.messageService.add({ severity: 'error', summary: 'Error', detail: err.Message });
      ...
      this.service.updateDiametricsFee(this.plan)
        .subscribe(data1 => {
          ...
          this.service.addLogs('DimetricsFees', data1.Id, localStorage.getItem('name'), localStorage.getItem('userId'), 'Update', old[0], data1)
          .subscribe(l => {
          });
        }, err => {
          this.messageService.add({ severity: 'error', summary: 'Error', detail: err.Message });
